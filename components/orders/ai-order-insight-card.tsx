...
export function AIOrderInsightCard({
  title,
  description,
  estimatedImpact,
  actions,
    ...
    <div className={cn("relative overflow-hidden rounded-[18px] border border-primary/20 bg-gradient-to-br from-primary/5 to-background p-6 shadow-sm", className)}>
      ...
      <div className="relative">
        ...
        <div className="mb-4 rounded-xl bg-danger/5 border border-danger/10 p-4">
          <p className="text-sm font-medium text-danger">{estimatedImpact}</p>
          <p className="text-xs text-muted-foreground">Potential revenue impact</p>
        </div>
        ...
        {impact && (
          <div className="mb-4 rounded-xl bg-success/5 border border-success/10 p-3">
            ...
            <p className="text-xs text-muted-foreground">Expected outcome if actions are taken</p>
          </div>
        )}
